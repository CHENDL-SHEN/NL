...
"""
DP与HD不同在于:
HD的（即本py中） RandomCrop_For_Segmentation_HD 是0，DP是255，
这是因为HDloss要将分割的像素级标签手动转换成0-1 onehot编码形式，因此pselabel中有255时会报错，
所以把pselabel中填充的像素标签设置为了0，
...
class RandomResize_For_Segmentation_HD:
    def __init__(self, min_image_size, max_image_size):
        self.min_image_size = min_image_size
        self.max_image_size = max_image_size
    ...
    def __call__(self, data):
        image, mask, maskB = data['image'], data['mask'], data['maskB']
        ...
        if size[0] == w and size[1] == h:
            ...
            data['maskB'] = maskB.resize(size, Image.NEAREST)
...
class RandomHorizontalFlip_For_Segmentation_HD:
    def __init__(self):
        pass
    ...
    def __call__(self, data):
        ...
        if bool(random.getrandbits(1)):
            ...
            data['maskB'] = maskB.transpose(Image.FLIP_LEFT_RIGHT)
...
class Normalize_For_Segmentation_HD:
    def __init__(self, mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225)):
        self.mean = mean
        self.std = std
    ...
    def __call__(self, data):
        ...
        maskB = np.asarray(maskB, dtype=np.int64)
...
class Normalize_For_Segmentation_VAL_HD:
    def __init__(self, mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225)):
        self.mean = mean
        self.std = std
...
class Top_Left_Crop_For_Segmentation_HD:
    def __init__(self, crop_size, channels=3):
        self.bg_value = 0
        self.crop_size = crop_size
        self.crop_shape = (self.crop_size, self.crop_size, channels)
...
class RandomCrop_For_Segmentation_HD(RandomCrop):
    ...
    def __call__(self, data):
        ...
        cropped_maskB = np.ones(self.crop_shape_for_mask, maskB.dtype) * self.bg_value
        cropped_maskB[dst_bbox['ymin']:dst_bbox['ymax'], dst_bbox['xmin']:dst_bbox['xmax']] = \
            maskB[src_bbox['ymin']:src_bbox['ymax'], src_bbox['xmin']:src_bbox['xmax']]

        data['image'] = cropped_image
        data['mask'] = cropped_mask
        data['maskB'] = cropped_maskB
        data['dst_bbox'] = dst_bbox
        data['src_bbox'] = src_bbox
...
class Transpose_For_Segmentation_HD:
    def __init__(self):
        pass
...
if __name__ == '__main__':
    pseB_path = '/media/ders/XS/PuzzleCAM-master-sel/experiments/predictions/binary@AffinityNet@ResNeSt-101@Puzzle@train@beta=10@exp_times=8@rw@crf=1/'
    pse_path = '/media/ders/XS/PuzzleCAM-master-sel/experiments/predictions/AffinityNet@ResNeSt-101@Puzzle@train@beta=10@exp_times=8@rw@crf=1/'
    image_path = '/media/ders/XS/dataset/VOC2012/JPEGImages/'

    id = '2007_000032'

    # pse_binary = torch.from_numpy(np.asarray(Image.open(pse_binary_path + '%s.png' % id))).long().cuda()
    # pse = torch.from_numpy(np.asarray(Image.open(pse_path + '%s.png' % id))).long().cuda()

    pseB = Image.open(pseB_path + '%s.png' % id)
    pse = Image.open(pse_path + '%s.png' % id)
    image = Image.open(image_path + '%s.jpg' % id)
    ...
    # RandomResize_For_Segmentation(50, 100)(data)
    # RandomHorizontalFlip_For_Segmentation()(data)
    # Normalize_For_Segmentation([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])(data)
    # RandomCrop_For_Segmentation(512)(data)
    # Transpose_For_SegmentationB()(data)
